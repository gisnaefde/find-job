...
const Sidebar = (props) => {
    ...
    const menus = [
        ...
        { title: "Tambah" , to:"/login"},
        { title: "Setting", to:"/changePassword/:id"}
    ]
        ...
        <div className="flex font-Mulish">
            <div className={`${open ? "w-72" : "w-0"} bg-white shadow-2xl h-screen relative duration-300`}>
                <div className="flex py-5 pl-9  bg-white border-b-4">
                    ...
                    <button className={`${open ? "absolute right-7" : "absolute -right-10"}  p-2 text-white absolute -right-12 text-xl font-bold bg-abu duration-300`} onClick={() => { setOpen(!open) }}><BiMenu/></button>
                </div>
